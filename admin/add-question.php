...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf($csrf)) {
        ...
        if ($qText === '' || $optA === '' || $optB === '' || $optC === '' || $optD === '') {
            $error = "Please fill in the question text and all four answer options.";
            ...
            try {
                $stmt = $db->prepare("
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$qText, $optA, $optB, $optC, $optD, $correct, $orderNum, $isActive]);
...
<head>
  ...
  <title>Add Question | Know Ali Admin</title>
        ...
        <form action="add-question.php" method="POST">
          ...
          <div class="form-group">
            ...
            <textarea 
              ...
              rows="3" 
              class="form-textarea" 
              placeholder="e.g. What is Ali's favorite programming language?" 
              required
            ><?= h($_POST['question_text'] ?? '') ?></textarea>
          </div>

          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 20px;">
            <div class="form-group" style="margin-bottom: 0;">
              <label for="option_a" class="form-label">Option A <span style="color: var(--primary-light);">*</span></label>
              <input type="text" name="option_a" id="option_a" class="form-input" placeholder="Option A text" value="<?= h($_POST['option_a'] ?? '') ?>" required>
            </div>

            <div class="form-group" style="margin-bottom: 0;">
              <label for="option_b" class="form-label">Option B <span style="color: var(--primary-light);">*</span></label>
              <input type="text" name="option_b" id="option_b" class="form-input" placeholder="Option B text" value="<?= h($_POST['option_b'] ?? '') ?>" required>
            </div>

            <div class="form-group" style="margin-bottom: 0;">
              <label for="option_c" class="form-label">Option C <span style="color: var(--primary-light);">*</span></label>
              <input type="text" name="option_c" id="option_c" class="form-input" placeholder="Option C text" value="<?= h($_POST['option_c'] ?? '') ?>" required>
            </div>

            <div class="form-group" style="margin-bottom: 0;">
              <label for="option_d" class="form-label">Option D <span style="color: var(--primary-light);">*</span></label>
              <input type="text" name="option_d" id="option_d" class="form-input" placeholder="Option D text" value="<?= h($_POST['option_d'] ?? '') ?>" required>
            </div>
          </div>
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 24px;">
            ...
            <div class="form-group" style="margin-bottom: 0;">
              ...
              <input type="number" name="order_num" id="order_num" class="form-input" value="<?= (int)($_POST['order_num'] ?? 0) ?>" min="0">
            </div>
          </div>

          <div class="form-group" style="display: flex; align-items: center; gap: 10px; margin-bottom: 28px;">
            <input type="checkbox" name="is_active" id="is_active" value="1" <?= (!isset($_POST['is_active']) || $_POST['is_active']) ? 'checked' : '' ?> style="width: 18px; height: 18px; accent-color: var(--primary);">
          ...
          <div style="display: flex; gap: 12px;">
            ...
            <button type="submit" class="btn btn-primary" style="flex: 2;">Save Question</button>
          </div>
        </form>
      </div>
    </main>
